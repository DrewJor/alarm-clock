...
//  Buzzer
//
//  Milestone 03 asks for three selectable sounds. A single-tone element
//  makes them by PATTERN rather than timbre.
// =====================================================================
...
bool hw_sound_ok() { return s_sound_ok; }
bool hw_light_ok() { return s_light_ok; }
static void buzzer_begin() {
  ...
  if (s_sound_ok) s_sound_ok = ledcWrite(PIN_BUZZER, 0);
  if (BUZZ_IS_LOW_IMPEDANCE)
    gpio_set_drive_capability((gpio_num_t)PIN_BUZZER, GPIO_DRIVE_CAP_0);
  Serial.println(s_sound_ok ? "[sound] output enabled; ready for user test" : "[sound] driver initialization failed");
}
static void buzzer_off() {
  if (s_sound_attached && !ledcWrite(PIN_BUZZER, 0)) {
    ledcDetach(PIN_BUZZER); s_sound_attached = false; s_sound_ok = false;
    pinMode(PIN_BUZZER, OUTPUT); digitalWrite(PIN_BUZZER, LOW);
  }
  s_last_frequency = s_last_duty = UINT32_MAX;
}
static bool buzzer_service(uint8_t sound, uint32_t el) {
  if (!s_sound_ok || sound < 1 || sound > 3) return false;
  uint32_t frequency = sound == 2 ? ((el / 150) % 2 ? 2400 : 1600) : 2000;
  uint32_t duty = sound == 1 && (el / 500) % 2 ? 0 : BUZZ_DUTY;
  if (sound == 3) duty = min(el / 120, uint32_t(BUZZ_DUTY));
  if (frequency != s_last_frequency) {
    s_sound_ok = ledcWriteTone(PIN_BUZZER, frequency) != 0;
    s_last_frequency = frequency; s_last_duty = UINT32_MAX;
  }
  if (s_sound_ok && duty != s_last_duty) {
    s_sound_ok = ledcWrite(PIN_BUZZER, duty); s_last_duty = duty;
  }
  if (!s_sound_ok) buzzer_off();
  return s_sound_ok;
}

// Alarm lighting now belongs to the OLED. The board RGB LED is cleared
...
static void light_off() { s_screen_pattern = 0; s_screen_elapsed = 0; }
static bool light_service(uint8_t pattern, uint32_t el) {
  if (pattern < 1 || pattern > 3) return s_light_ok = false;
  if (!s_screen_pattern) {
    Wire.beginTransmission(OLED_ADDR);
    s_light_ok = Wire.endTransmission() == 0;
    if (!s_light_ok) return false;
  }
  s_screen_pattern = pattern; s_screen_elapsed = el;
  return true;
}
void hw_display_present() {
  if (s_screen_pattern && screen_effect(s_screen_pattern, s_screen_elapsed).inverted) {
    ...
    for (size_t i = 0; i < count; ++i) pixels[i] ^= 0xff;
  }
  oled.sendBuffer();
}
...
static void ldr_begin() {
  ...
  analogSetPinAttenuation(PIN_LDR, static_cast<adc_attenuation_t>(ADC_ATTEN_DB_12));      // ~0-3.1 V window
  (void)analogRead(PIN_LDR);
...
uint16_t hw_ldr_raw() {
  uint32_t acc = 0;
  for (int i = 0; i < 16; i++) acc += analogRead(PIN_LDR);
...
void    hw_brightness_apply(uint8_t level, bool automatic) {
  ...
  uint8_t base_contrast = level >= 3 ? 255 : level == 2 ? (automatic ? 40 : 110)
                                                                       : (automatic ? 0 : 8);
  uint8_t contrast = s_screen_pattern ? screen_effect(s_screen_pattern, s_screen_elapsed).contrast
                                     : base_contrast;
  if (contrast == s_oled_contrast) return;
  oled.setContrast(contrast);
  s_oled_contrast = contrast;
}
uint8_t hw_brightness_level() { return s_level; }

// =====================================================================
//  Clock access - Timekeeping is the sole RTC writer (chart 8)
// =====================================================================
...
static bool rtc_registers(uint8_t reg, uint8_t *out, uint8_t count) {
  ...
  if (Wire1.requestFrom(uint8_t(RTC_ADDR), count) != count) return false;
  for (uint8_t i = 0; i < count; ++i) out[i] = Wire1.read();
  return true;
}
static bool bcd(uint8_t v, uint8_t &out) {
  if ((v & 15) > 9 || (v >> 4) > 9) return false;
  out = (v >> 4) * 10 + (v & 15); return true;
}
static bool rtc_sample(DateTime &out) {
  ...
  s_clock_valid = false;
  if (was != s_rtc_ok) Serial.println(s_rtc_ok ? "[rtc] reconnected" : "[rtc] Clock unavailable");
  if (!s_rtc_ok || (status & 0x80)) return false;
  uint8_t se, mi, ho, day, month, year;
  if (!bcd(r[0] & 0x7f, se) || !bcd(r[1] & 0x7f, mi) ||
      !bcd(r[2] & ((r[2] & 0x40) ? 0x1f : 0x3f), ho) ||
      !bcd(r[4] & 0x3f, day) || !bcd(r[5] & 0x1f, month) || !bcd(r[6], year)) return false;
  if (r[2] & 0x40) { if (ho < 1 || ho > 12) return false; ho = ho % 12 + ((r[2] & 0x20) ? 12 : 0); }
  if ((r[5] & 0x80) || se > 59 || mi > 59 || ho > 23 || month < 1 || month > 12 ||
      day < 1 || day > days_in_month(2000 + year, month)) return false;
  out = DateTime(2000 + year, month, day, ho, mi, se);
  s_clock_valid = true;
  return true;
}
DateTime hw_rtc_read() { return s_standard; }
bool hw_clock_available() { return s_rtc_ok; }
bool hw_clock_valid() { return s_clock_valid; }
int8_t hw_dst_offset() { return s_dst_off; }
bool hw_rtc_write(const DateTime &standard) {
  if (!standard.isValid() || standard.year() < YEAR_MIN || standard.year() > YEAR_MAX) return false;
  uint8_t status;
  if (!rtc_registers(0x0f, &status, 1)) return false;
  // rtc.begin can have failed at boot; initialize its bus device on reconnect.
  if (!rtc.begin(&Wire1)) return false;
  rtc.adjust(standard);
  DateTime rb;
  if (!rtc_sample(rb)) return false;
  int64_t delta = int64_t(rb.unixtime()) - standard.unixtime();
  if (delta < 0 || delta > 2) return false;
  s_standard = rb; s_dst_off = dst_offset_hours(rb); s_local = to_local(rb);
  schedule.discontinuity();
  Serial.printf("[rtc] verified standard=%lu\n", (unsigned long)rb.unixtime());
  return true;
}
static bool save_read(DateTime &value) { return rtc_sample(value); }
...
static bool save_format(bool fmt24h) {
  bool old = cfg.fmt24h; cfg.fmt24h = fmt24h;
  if (settings_save()) return true;
  cfg.fmt24h = old; return false;
}
static const SaveBackend save_backend = {save_read, hw_rtc_write, save_format};

// =====================================================================
//  Previews - sections 11 and 12. A test never changes a saved setting.
// =====================================================================
...
bool hw_sound_test(uint8_t s) {
  hw_test_stop();
  if (!buzzer_service(s, 0)) return false;
  s_test_kind = 1; s_test_which = s; s_test_start = millis(); return true;
}
bool hw_light_test(uint8_t l) {
  hw_test_stop();
  if (!light_service(l, 0)) return false;
  s_test_kind = 2; s_test_which = l; s_test_start = millis(); return true;
}
bool hw_testing()             { return s_test_kind != 0; }
void hw_test_stop() {
  if (!s_test_kind) return;
  s_test_kind = 0;
  buzzer_off();
  light_off();
}
...
// =====================================================================
//  Alarm runtime - sections 9 and 13
// =====================================================================
...
static bool snooze_remaining(uint8_t index, uint32_t now, uint32_t &seconds) {
  seconds = 0;
  if (index >= 3 || !s_snoozing[index] || !cfg.alarm[index].enabled) return false;
...
bool hw_next_alarm(DateTime &local, uint8_t &index) {
  if (!s_clock_valid) return false;
  bool found = schedule.next(s_standard, cfg, local, index);
  for (uint8_t i = 0; i < 3; ++i) {
    if (!s_snoozing[i] || !cfg.alarm[i].enabled) continue;
    ...
    if (!found || next.unixtime() < local.unixtime()) { local = next; index = i; found = true; }
  }
  return found;
}

bool    hw_alarm_ringing()       { return s_ringing; }
uint8_t hw_alarm_index()         { return s_ring_idx; }
uint8_t hw_alarm_snoozes_used()  { return s_snz_used[s_ring_idx]; }

static void alarm_start(uint8_t i, bool from_snooze) {
  hw_test_stop();
  if (!from_snooze) s_snz_used[i] = 0;
  s_snoozing[i] = false;
  s_ringing = true; s_ring_idx = i; s_ring_ms = millis();
...
// Snooze: allowed only while snoozes remain (section 13).
bool hw_alarm_snooze() {
  if (!s_ringing) return false;
  uint8_t i = s_ring_idx;
  if (s_snz_used[i] >= cfg.alarm[i].snooze_max) return false;
  s_snz_used[i]++;
  ...
  Serial.printf("[alarm] %u snoozed %u min (%u of %u)\n", i + 1,
                cfg.alarm[i].snooze_min, s_snz_used[i], cfg.alarm[i].snooze_max);
  return true;
}
...
void hw_alarm_settings_changed(uint8_t i) {
  s_snoozing[i] = false; s_pending_alarms &= ~(1 << i);
}
static void service_due_alarms(uint32_t now) {
  if (s_ringing) return;
  for (uint8_t i = 0; i < 3; ++i) {
    if (!cfg.alarm[i].enabled) { s_snoozing[i] = false; s_pending_alarms &= ~(1 << i); continue; }
    if (s_snoozing[i] && int32_t(now - s_snz_due[i]) >= 0) { alarm_start(i, true); return; }
    if (s_pending_alarms & (1 << i)) {
      s_pending_alarms &= ~(1 << i); alarm_start(i, false); return;
    }
  }
}

static void cancel_snoozes() {
  for (uint8_t i = 0; i < 3; ++i) s_snoozing[i] = false;
...
static bool alarm_input(InputEvent ev) {
  if (ev == EV_SNOOZE_PRESS || ev == EV_SNOOZE_HOLD) {
    if (s_ringing && !hw_alarm_snooze()) {
      Serial.println("[alarm] no snoozes left"); ui_snooze_refused();
    } else if (!s_ringing) {
  ...
  if (!s_ringing) return false;
  if (ev == EV_BACK_PRESS || ev == EV_BACK_HOLD) hw_alarm_dismiss();
...
static bool alarm_timeout(uint32_t now) {
  if (s_ringing && cfg.alarm[s_ring_idx].length_min != 0 &&
    ...
    Serial.println("[alarm] length expired"); hw_alarm_dismiss(); return true;
  }
  return false;
}
#if SELFTEST_RULES
static void runtime_selftest() {
  ...
  for (auto &a : cfg.alarm) { a.enabled = true; a.snooze_max = 1; a.snooze_min = SNOOZE_DELAY_MIN; a.length_min = 1; }
  int checks = 0, failed = 0;
  auto check = [&](bool ok, const char *name) { ++checks; if (!ok) { ++failed; Serial.printf("[test] FAIL %s\n", name); } };
  alarm_start(0, false);
  ...
  s_snz_due[0] = millis(); service_due_alarms(millis());
  ...
  hw_alarm_dismiss(); check(!s_ringing && !s_snoozing[0], "dismiss clears session");
  s_pending_alarms = 7; service_due_alarms(millis());
  check(s_ring_idx == 0 && s_pending_alarms == 6, "simultaneous queue starts first");
  hw_alarm_dismiss(); service_due_alarms(millis());
  ...
  service_due_alarms(millis()); check(s_ring_idx == 2 && s_pending_alarms == 0, "simultaneous queue starts third");
  hw_alarm_snooze(); cancel_snoozes();
  check(!s_snoozing[2], "explicit bench cleanup cancels waiting session");
  alarm_start(0, false); hw_alarm_snooze(); cfg.alarm[0].enabled = false;
  service_due_alarms(millis()); check(!s_snoozing[0], "disabled alarm cancels snooze");
  for (uint8_t minutes : {uint8_t(5), uint8_t(9), uint8_t(10), uint8_t(15)}) {
    cfg.alarm[0].enabled = true; cfg.alarm[0].snooze_min = minutes;
    alarm_start(0, false);
  ...
  alarm_start(0, false);
  check(!alarm_timeout(s_ring_ms + 59999UL) && s_ringing, "finite alarm stays active before its deadline");
  ...
  alarm_silence();
  cfg.alarm[0].daily = false; cfg.alarm[0].snooze_max = 3;
  alarm_start(0, false);
  ...
  alarm_input(EV_SNOOZE_HOLD); alarm_input(EV_SNOOZE_PRESS);
  ...
  s_snz_due[0] = 500;
  check(snooze_remaining(0, UINT32_MAX - 499, seconds) && seconds == 1, "countdown across millis wrap");
  s_snz_due[0] = due;
  ...
  alarm_silence(); s_pending_alarms = 0;
  for (uint8_t i = 0; i < 3; ++i) { s_snoozing[i] = false; s_snz_used[i] = 0; s_snz_due[i] = 0; }
  cfg = saved;
...
// =====================================================================
struct PinCheck { uint8_t pin; const char *name; const char *expect; };
...
static void wiring_report() {
  ...
  bool shorted = false;
  for (auto &c : CHECKS) {
    pinMode(c.pin, INPUT_PULLUP);   delay(4); int up = digitalRead(c.pin);
    pinMode(c.pin, INPUT_PULLDOWN); delay(4); int dn = digitalRead(c.pin);
    pinMode(c.pin, INPUT_PULLUP);
    const char *st = (up == 1 && dn == 0) ? "floating " : (up == 0) ? "LOW      " : "HIGH     ";
    Serial.printf("[wire] %s  %s  %s\n", c.name, st, c.expect);
    if (up == 0 && (c.pin == PIN_BTN_BACK ||
                    c.pin == PIN_BTN_SNOOZE || c.pin == PIN_ENC_SW)) shorted = true;
  }
  if (shorted) {
...
static void i2c_report() {
  struct { TwoWire *w; const char *name; uint8_t expect; } bus[] = {
...
static void ldr_self_test() {
  ...
  if      (v > 4000) Serial.println("PINNED HIGH - pull-down is not engaging.");
  else if (v < 30)   Serial.println("near zero - shine a light and re-check.");
  else               Serial.println("plausible. Log dark / room / torch and set "
...
void setup() {
  ...
  if (!s_rtc_ok) Serial.println("[rtc] DS3231 not found on bus 1");
  // Deliberately NOT calling rtc.adjust() here. The DS3231's oscillator-stop
  // flag is the authority on whether its time means anything, and Milestone
  // 03 chart 1 says an unreadable clock must ask to be set rather than show
  // a fabricated time. Compile time is a fabricated time.
  ...
  hw_brightness_apply(cfg.auto_bright ? ldr_level(hw_ldr_raw()) : cfg.manual_level, cfg.auto_bright);
  if (rtc_sample(s_standard)) { s_local = to_local(s_standard); s_dst_off = dst_offset_hours(s_standard); }     // so the first frame is not 2000-01-01
}
...
static InputEvent bench_poll() {
  static char line[40]; static uint8_t used = 0;
  while (Serial.available()) {
    ...
    struct { const char *name; InputEvent event; } events[] = {
      {"cw", EV_ENC_CW}, {"ccw", EV_ENC_CCW}, {"select", EV_ENC_PRESS},
      {"back", EV_BACK_PRESS}, {"alarm", EV_ALARM_PRESS}, {"snooze", EV_SNOOZE_PRESS},
      {"holdsnooze", EV_SNOOZE_HOLD}, {"holdback", EV_BACK_HOLD}
    };
    for (auto &e : events) if (!strcmp(line, e.name)) return e.event;
    if (!strcmp(line, "status")) {
      Serial.printf("[status] available=%d valid=%d standard=%lu local=%lu dst=%d ldr=%u brightness=%u sound=%d light=%d ringing=%d pending=%u fmt24=%d\n",
        s_rtc_ok, s_clock_valid, (unsigned long)s_standard.unixtime(), (unsigned long)s_local.unixtime(),
        s_dst_off, hw_ldr_raw(), s_level, s_sound_ok, s_light_ok, s_ringing, s_pending_alarms, cfg.fmt24h);
      bool automatic = cfg.auto_bright;
      uint8_t manual_level = cfg.manual_level;
      ui_brightness_mode(automatic, manual_level);
      Serial.printf("[brightness] saved_auto=%d active_auto=%d manual=%u applied=%u thresholds=%u/%u hysteresis=%u\n",
        cfg.auto_bright, automatic, manual_level, s_level,
        LDR_THRESH_LO, LDR_THRESH_HI, LDR_HYSTERESIS);
      Serial.printf("[screen] pattern=%u inverted=%d contrast=%d\n", s_screen_pattern,
        s_screen_pattern && screen_effect(s_screen_pattern, s_screen_elapsed).inverted, s_oled_contrast);
      Serial.printf("[buttons] snooze18=%d back14=%d select12=%d (1=released, 0=pressed)\n",
        digitalRead(PIN_BTN_SNOOZE), digitalRead(PIN_BTN_BACK), digitalRead(PIN_ENC_SW));
      for (uint8_t i = 0; i < 3; ++i) {
        int32_t remaining = s_snoozing[i] ? int32_t(s_snz_due[i] - millis()) : 0;
        Serial.printf("[alarm-status] %u enabled=%d %02u:%02u daily=%d snoozing=%d delay_min=%u remaining_ms=%lu ring_min=%u max_snoozes=%u\n",
          i + 1, cfg.alarm[i].enabled, cfg.alarm[i].hour, cfg.alarm[i].minute, cfg.alarm[i].daily, s_snoozing[i],
          cfg.alarm[i].snooze_min, (unsigned long)(remaining > 0 ? remaining : 0), cfg.alarm[i].length_min, cfg.alarm[i].snooze_max);
      }
    } else if (!strcmp(line, "stop")) {
      hw_alarm_dismiss(); cancel_snoozes(); hw_test_stop();
    } else if (!strcmp(line, "ldr")) ldr_self_test();
    else if (!strncmp(line, "ring-test ", 10) && !s_ringing) {
      int i = atoi(line + 10); if (i >= 1 && i <= 3) alarm_start(i - 1, false);
    }
    else if (!strncmp(line, "home-preview ", 13)) {
      unsigned mask, format;
      if (sscanf(line + 13, "%u %u", &mask, &format) == 2 && mask <= 7 && (format == 12 || format == 24)) {
        ui_home_preview(mask, format == 24); bench_frame();
      }
    } else if (!strcmp(line, "frame")) {
      bench_frame();
    } else if (!strncmp(line, "sound ", 6) && !s_ringing) {
      if (!hw_sound_test(atoi(line + 6))) ui_output_error(true);
    } else if (!strncmp(line, "light ", 6) && !s_ringing) {
      if (!hw_light_test(atoi(line + 6))) ui_output_error(false);
    } else Serial.println("[bench] status frame ldr cw ccw select back alarm snooze stop sound 1..3 light 1..3");
  }
  return EV_NONE;
}
#endif
...
void loop() {
  ...
  if (now - last_tick >= TICK_MS) {
    ...
    if (rtc_sample(s_standard)) {
      ...
      if (offset != s_dst_off) Serial.printf("[dst] offset %d -> %d; scheduler notified\n", s_dst_off, offset);
      s_dst_off = offset;
      s_local = s_standard + TimeSpan(offset * 3600);
      s_pending_alarms |= schedule.poll(s_standard, cfg);
    } else schedule.discontinuity();
  }
...
#if BENCH_SERIAL
  if (ev == EV_NONE) ev = bench_poll();
#endif
  if (ev != EV_NONE) {
#if LOG_INPUT
    Serial.printf("[input] %s\n", input_name(ev));
#endif
    if (!alarm_input(ev)) ui_event(ev);
  }
  ...
  if (s_ringing) {
    ...
    // Alarm length expiring counts as the alarm giving up (section 13).
  ...
  // ---- preview ---------------------------------------------------------
    ...
    if (el >= TEST_MS) {
      ...
      if (!buzzer_service(s_test_which, el)) { hw_test_stop(); ui_output_error(true); }
    } else {
      if (!light_service(s_test_which, s_test_which == 3 ? el * 15 : el)) { hw_test_stop(); ui_output_error(false); }
    }
  }
