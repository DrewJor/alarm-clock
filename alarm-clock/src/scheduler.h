...
class AlarmSchedule {
 public:
  ...
  bool next(const DateTime &standard, const Settings &settings,
            DateTime &local, uint8_t &index) const;
  void discontinuity() { have_previous = false; }
 private:
...
bool dst_adjustment(const DateTime &standard, const Settings &settings,
                    int8_t direction, int8_t &offset);
...
bool local_to_standard(const DateTime &local, const Settings &settings,
                       DateTime &proposed);
