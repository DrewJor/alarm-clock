...
//  Rotary encoder
//
//  Decoded by the PCNT peripheral, not by an interrupt. PCNT counts the
//  quadrature pair in hardware and has a glitch filter that discards any
//  pulse shorter than ENC_GLITCH_NS, which is what rejects contact bounce.
//  A bare edge ISR on a mechanical encoder double-counts; this does not,
//  and it costs no CPU.
// ---------------------------------------------------------------------
...
static void encoder_begin() {
  ...
  ca.edge_gpio_num = PIN_ENC_A;  ca.level_gpio_num = PIN_ENC_B;
  ...
  cb.edge_gpio_num = PIN_ENC_B;  cb.level_gpio_num = PIN_ENC_A;
...
static void encoder_pump() {
  int raw = 0;
  if (pcnt_unit_get_count(s_unit, &raw) != ESP_OK) return;
...
static InputEvent button_poll(Button &b, uint32_t now) {
  ...
  if (r != b.last_reading) { b.last_reading = r; b.changed_ms = now; return EV_NONE; }
  if (now - b.changed_ms < DEBOUNCE_MS) return EV_NONE;   // still settling

  bool low = (r == LOW);
  if (!b.armed) { if (!low) b.armed = true; return EV_NONE; }                                   // LOW = pressed
  if (low && !b.stable_low) {                              // just pressed
    b.stable_low = true; b.down_ms = now; b.hold_fired = false;
    ...
    b.hold_fired = true; return b.ev_hold;                 // held long enough
  } else if (!low && b.stable_low) {                       // just released
    b.stable_low = false;
    if (!b.hold_fired) return b.ev_press;                  // short press
  }
  return EV_NONE;
}
...
// ---------------------------------------------------------------------
void input_begin() {
  for (int i = 0; i < N_BTN; i++) pinMode(s_btn[i].pin, INPUT_PULLUP);
  encoder_begin();
}
...
InputEvent input_poll() {
  ...
  if (s_pending > 0) { s_pending--; return EV_ENC_CW; }
  if (s_pending < 0) { s_pending++; return EV_ENC_CCW; }
  return EV_NONE;
}

const char* input_name(InputEvent e) {
  switch (e) {
    case EV_ENC_CW:       return "ENC_CW";
    case EV_ENC_CCW:      return "ENC_CCW";
    case EV_ENC_PRESS:    return "ENC_PRESS";
    case EV_ENC_HOLD:     return "ENC_HOLD";
    case EV_ALARM_PRESS:  return "ALARM";
    case EV_ALARM_HOLD:   return "ALARM_HOLD";
    case EV_SNOOZE_PRESS: return "SNOOZE";
    case EV_SNOOZE_HOLD:  return "SNOOZE_HOLD";
    case EV_BACK_PRESS:   return "BACK";
    case EV_BACK_HOLD:    return "BACK_HOLD";
    default:              return "NONE";
  }
}
