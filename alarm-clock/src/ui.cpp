...
//
//  Implements Milestone 03 software sections 1-14. Structure follows the
//  flowcharts: every editor works on a DRAFT, and the draft only reaches
//  saved settings through an explicit Save.
//
...
enum Screen : uint8_t {
  SCR_HOME,          // 1
  SCR_MENU,          // 2
  SCR_TD_MENU,       // 3   set time/date submenu
  SCR_TD_TIME,       // 4   time field list
  SCR_TD_DATE,       // 5   date field list
  SCR_TD_SAVING, SCR_TD_REVIEW,
  SCR_TD_CONFIRM,    // 7   save / discard / continue
  SCR_AL_LIST,       // 9   alarm 1 / 2 / 3
  SCR_AL_MENU,       // 9   individual alarm menu
  SCR_AL_DATEOPT,    // 10  daily or specific date
  SCR_AL_DATE,       // 10
  SCR_AL_TIME,       // 10
  SCR_AL_SOUND,      // 11
  SCR_AL_SOUND_OPT,  // 11  set / test
  SCR_AL_LIGHT,      // 12
  SCR_AL_LIGHT_OPT,  // 12
  SCR_AL_SNOOZE,     // 13
  SCR_AL_CONFIRM,    // 13  save / discard / continue
  SCR_BRIGHT,        // 13  brightness editor
  SCR_BRIGHT_CONFIRM,
  SCR_DST,           // 14
  SCR_DST_CONFIRM,
...
// ---------------------------------------------------------------------
//  Navigation stack.  go() remembers the row we left, so returning from a
//  child screen restores it - charts 2 and 3 both require that.
// ---------------------------------------------------------------------
struct NavEntry { uint8_t scr, cur; };
static NavEntry nav[8];
static uint8_t  nav_sp = 0;

static void go(Screen s) {
  if (nav_sp < 8) nav[nav_sp++] = { (uint8_t)scr, cur };
  scr = s; cur = 0; editing = false;
}
static void go_replace(Screen s) { scr = s; cur = 0; editing = false; }
static void go_back() {
  editing = false;
  if (nav_sp) { NavEntry e = nav[--nav_sp]; scr = (Screen)e.scr; cur = e.cur; }
  else        { scr = SCR_HOME; cur = 3; }
}
static void go_home() { nav_sp = 0; scr = SCR_HOME; cur = 3; editing = false; }
static void pop_to(Screen s) {
  editing = false;
  while (nav_sp && scr != s) { NavEntry e = nav[--nav_sp]; scr = (Screen)e.scr; cur = e.cur; }
  if (scr != s) { scr = s; cur = 0; }
}
...
static void msg_show(const char *l1, const char *l2, Screen next, uint8_t next_cur) {
  ...
  msg_next = next; msg_next_cur = next_cur;
  msg_until = millis() + MSG_MS; msg_active = true;
  scr = SCR_MSG; editing = false;
}
...
//  Drafts
// ---------------------------------------------------------------------
// Clock time/date draft (charts 3-8). These are LOCAL values, i.e. what
// the user reads off the screen; the DST offset is removed on save.
struct TD { uint16_t year; uint8_t month, day, hour, minute, second; bool fmt24h; };
static TD   td, td_orig, td_snap;
static bool td_reviewed_time = false;   // chart 3: first setup must review
static bool td_reviewed_date = false;
static bool td_day_adjusted  = false;   // chart 5: show "Day adjusted"

// Alarm draft (charts 9-13)
...
static bool    br_auto_orig;  static uint8_t br_level_orig;
static bool    dst_auto_orig; static int8_t  dst_hr_orig;
...
static bool next_snooze(uint8_t &index, uint32_t &seconds) {
  ...
  for (uint8_t i = 0; i < 3; ++i) {
    ...
    if (hw_snooze_remaining(i, left) && (!found || left < seconds)) {
      index = i; seconds = left; found = true;
    }
  }
  return found;
}
...
static bool alarm_equal(const Alarm &a, const Alarm &b) {
  return a.enabled==b.enabled && a.hour==b.hour && a.minute==b.minute &&
         a.daily==b.daily && a.year==b.year && a.month==b.month &&
         a.day==b.day && a.sound==b.sound && a.light==b.light &&
         a.snooze_min==b.snooze_min && a.snooze_max==b.snooze_max &&
         a.length_min==b.length_min;
}

// 24-hour hour -> the digits a 12-hour display shows (chart 4).
static uint8_t hour12(uint8_t h24) { uint8_t h = h24 % 12; return h ? h : 12; }
static bool    is_pm (uint8_t h24) { return h24 >= 12; }
// Put a 12-hour digit and an AM/PM flag back together.
static uint8_t hour24(uint8_t h12, bool pm) { return (uint8_t)(h12 % 12) + (pm ? 12 : 0); }

// =====================================================================
//  Chart 6 - shared field editing
//
...
// =====================================================================
static uint8_t td_time_fields()  { return td.fmt24h ? 3 : 4; }   // H M S [AM/PM]
static uint8_t al_time_fields()  { return cfg.fmt24h ? 2 : 3; }  // H M   [AM/PM]

// After a year or month change the day may no longer exist. Reduce it to
// the last valid day and say so - chart 5, "Dependent day adjustment".
static void clamp_day(uint16_t y, uint8_t m, uint8_t &d, bool &flag) {
  uint8_t max = days_in_month(y, m);
  if (d > max) { d = max; flag = true; }
}

static void td_step(int dir) {
  switch (cur) {
    case 0:                                        // hour
      if (td.fmt24h) td.hour = wrapi(td.hour + dir, 0, 23);
      else           td.hour = hour24(wrapi(hour12(td.hour) + dir, 1, 12), is_pm(td.hour));
      break;
    case 1: td.minute = wrapi(td.minute + dir, 0, 59); break;
    case 2: td.second = wrapi(td.second + dir, 0, 59); break;
    case 3: td.hour = hour24(hour12(td.hour), !is_pm(td.hour)); break;   // AM/PM toggle
  }
}
...
static void al_time_step(int dir) {
  switch (cur) {
    case 0:
      if (cfg.fmt24h) al.hour = wrapi(al.hour + dir, 0, 23);
      else            al.hour = hour24(wrapi(hour12(al.hour) + dir, 1, 12), is_pm(al.hour));
      break;
    case 1: al.minute = wrapi(al.minute + dir, 0, 59); break;
    case 2: al.hour = hour24(hour12(al.hour), !is_pm(al.hour)); break;
  }
}
static void al_date_step(int dir) {
  static bool ignore = false;
  switch (cur) {
    case 0: al.year  = clampi(al.year + dir, YEAR_MIN, YEAR_MAX);
            clamp_day(al.year, al.month, al.day, ignore); break;
    case 1: al.month = wrapi(al.month + dir, 1, 12);
            clamp_day(al.year, al.month, al.day, ignore); break;
    case 2: al.day   = wrapi(al.day + dir, 1, days_in_month(al.year, al.month)); break;
  }
}
// Clamp at the limits: snooze delay is 5-15 minutes in one-minute steps.
static void al_snooze_step(int dir) {
  switch (cur) {
    case 0: al.snooze_min = clampi(al.snooze_min + dir, SNOOZE_DELAY_MIN, SNOOZE_DELAY_MAX); break;
    case 1: al.snooze_max = clampi(al.snooze_max + dir, SNOOZE_COUNT_MIN, SNOOZE_COUNT_MAX); break;
    case 2: al.length_min = clampi(al.length_min + dir, ALARM_LEN_MIN,    ALARM_LEN_MAX);    break;
  }
}

// =====================================================================
//  Chart 3 / 7 / 8 - set time/date
// =====================================================================
...
// Chart 7: "Pending first setup counts as unsaved time and date."
static bool td_dirty() {
  if (!hw_clock_valid()) return true;
...
static void td_save() {
  if (scr == SCR_TD_CONFIRM) go_back();
  if (td.year < YEAR_MIN || td.year > YEAR_MAX || td.month < 1 || td.month > 12 ||
      td.day < 1 || td.day > days_in_month(td.year, td.month) ||
      td.hour > 23 || td.minute > 59 || td.second > 59) {
    msg_show("Invalid date or time", "Nothing was saved", SCR_TD_MENU, cur); return;
  }
  if (!td_reviewed_time || !td_reviewed_date) {
    msg_show("First setup: open", "Time and Date first", SCR_TD_MENU, cur); return;
  }
  td_request = SaveRequest{};
  if (!hw_clock_valid() || td.hour != td_orig.hour || td.minute != td_orig.minute || td.second != td_orig.second)
    td_request.groups |= SAVE_TIME;
  if (!hw_clock_valid() || td.year != td_orig.year || td.month != td_orig.month || td.day != td_orig.day)
    td_request.groups |= SAVE_DATE;
  if (td.fmt24h != td_orig.fmt24h) td_request.groups |= SAVE_FORMAT;
...
// Chart 7. Only the submenu raises the prompt: charts 4 and 5 say Back
// from the Time or Date list returns to the submenu with the draft intact,
// so that move is still inside the editing session, not a request to leave.
static void td_request_leave() {
  if (!td_dirty()) { go_back(); return; }
  go(SCR_TD_CONFIRM);
...
static void al_enter(uint8_t i) {
  sel_alarm = i;
  al_parent = scr; al_parent_cur = cur;
  al = al_orig = cfg.alarm[i];
  go(SCR_AL_MENU);
}
static void al_save() {
  if (scr == SCR_AL_CONFIRM) go_back();
  if (al.hour > 23 || al.minute > 59) { msg_here("Invalid alarm time"); return; }
  if (!al.daily && (al.year < YEAR_MIN || al.year > YEAR_MAX ||
                    al.month < 1 || al.month > 12 ||
                    al.day < 1 || al.day > days_in_month(al.year, al.month))) {
    msg_here("Invalid alarm date"); return;
  }
  Alarm old = cfg.alarm[sel_alarm];
  cfg.alarm[sel_alarm] = al;
  if (!settings_save()) { cfg.alarm[sel_alarm] = old; msg_here("Alarm not saved", "Storage error"); return; }
  hw_alarm_settings_changed(sel_alarm);
  al_orig = al;
  char l2[24]; snprintf(l2, sizeof(l2), "Alarm %u", sel_alarm + 1);
...
static void al_request_leave() {
  if (alarm_equal(al, al_orig)) { go_back(); return; }
  go(SCR_AL_CONFIRM);
  cur = 2;
}

// =====================================================================
//  Chart 13 - brightness,  chart 14 - DST
// =====================================================================
...
static void bright_discard() {
  if (scr == SCR_BRIGHT_CONFIRM) go_back();
  cfg.auto_bright  = br_auto_orig;      // the level was applied live, so
  cfg.manual_level = br_level_orig;     // undoing it means re-applying
  if (!cfg.auto_bright) hw_brightness_apply(cfg.manual_level);
  go_back();
}
static void bright_save() {
  if (scr == SCR_BRIGHT_CONFIRM) go_back();
  cfg.auto_bright = br_auto_draft; cfg.manual_level = br_level_draft;
  if (!settings_save()) {
    cfg.auto_bright = br_auto_orig; cfg.manual_level = br_level_orig;
    msg_here("Brightness not saved", "Storage error");
    return;
  }
  go_back(); msg_here("Brightness saved");
}
...
static void dst_discard() {
  if (scr == SCR_DST_CONFIRM) go_back();
...
static void dst_save() {
  if (scr == SCR_DST_CONFIRM) go_back();
  if (dst_direction) {
    if (!hw_clock_valid()) { msg_here("Set clock first"); return; }
    ...
    if (!dst_adjustment(hw_rtc_read(), cfg, dst_direction, dst_hr_draft)) {
      msg_here("Hour limit reached"); return;
    }
  }
  cfg.dst_auto = dst_auto_draft; cfg.dst_manual_hr = dst_hr_draft;
  if (!settings_save()) {
    cfg.dst_auto = dst_auto_orig; cfg.dst_manual_hr = dst_hr_orig;
    msg_here("DST not saved", "Storage error"); return;
  }
  hw_dst_settings_changed();
  go_back(); msg_here("DST saved");
}

// =====================================================================
//  Event handling
// =====================================================================
static void move(int dir, uint8_t n) {           // charts 2-5: highlight wraps
  if (n) cur = (uint8_t)wrapi(cur + dir, 0, n - 1);
}

static void td_snap_save()    { td_snap = td; }
static void td_snap_restore() { td = td_snap; td_day_adjusted = false; }
static void al_snap_save()    { al_snap = al; }
static void al_snap_restore() { al = al_snap; }

// Chart 6, in one place. Returns true when the event belonged to an active
// value edit and has been consumed.
static bool edit_common(InputEvent e, void (*step)(int), void (*restore)()) {
  if (!editing) return false;
  if      (e == EV_ENC_CW)     step(+1);
  else if (e == EV_ENC_CCW)    step(-1);
  else if (e == EV_ENC_PRESS)  editing = false;          // keep the draft value
  else if (e == EV_BACK_PRESS) { restore(); editing = false; }
  return true;
}
...
void ui_event(InputEvent e) {
  ...
  if (overlay) return;
  if (snooze_countdown) {
    uint8_t index; uint32_t seconds;
    if (next_snooze(index, seconds)) {
      if (e == EV_BACK_PRESS || e == EV_BACK_HOLD || e == EV_ENC_PRESS ||
          e == EV_ENC_CW || e == EV_ENC_CCW || e == EV_ENC_HOLD)
  ...
  case SCR_HOME:
    if (e == EV_ENC_CW) move(+1, 4);
    else if (e == EV_ENC_CCW) move(-1, 4);
    else if (e == EV_ENC_PRESS) {
      if (cur < 3) al_enter(cur);
      else go(SCR_MENU);
    }
    break;
  case SCR_MENU:
    ...
    else if (e == EV_BACK_PRESS) go_back();
    else if (e == EV_ENC_PRESS) {
      switch (cur) {
        case 0: td_enter(); break;
        case 1: go(SCR_AL_LIST); break;
        case 2: bright_enter(); break;
        case 3: dst_enter(); break;
      }
    }
    break;
  case SCR_TD_MENU:
    if (e == EV_ENC_CW) move(+1, 5);
    else if (e == EV_ENC_CCW) move(-1, 5);
    else if (e == EV_BACK_PRESS) td_request_leave();
    else if (e == EV_ENC_PRESS) {
      switch (cur) {
        case 0: td.fmt24h = !td.fmt24h; break;
        case 1: td_reviewed_time = true; go(SCR_TD_TIME); break;
        case 2: td_reviewed_date = true; go(SCR_TD_DATE); break;
        case 3: td_save(); break;
        case 4: td_discard(); break;
      }
    }
    break;
  case SCR_TD_TIME:
    if (edit_common(e, td_step, td_snap_restore)) break;
    if (e == EV_ENC_CW) move(+1, td_time_fields());
    else if (e == EV_ENC_CCW) move(-1, td_time_fields());
    else if (e == EV_ENC_PRESS) { td_snap_save(); editing = true; }
    else if (e == EV_BACK_PRESS) go_back();
    break;
  case SCR_TD_DATE:
    if (edit_common(e, td_date_step, td_snap_restore)) break;
    if (e == EV_ENC_CW) { move(+1, 3); td_day_adjusted = false; }
    else if (e == EV_ENC_CCW) { move(-1, 3); td_day_adjusted = false; }
    else if (e == EV_ENC_PRESS) { td_snap_save(); editing = true; }
    else if (e == EV_BACK_PRESS) go_back();
    break;
  case SCR_TD_CONFIRM:
    if (e == EV_ENC_CW) move(+1, 3);
    else if (e == EV_ENC_CCW) move(-1, 3);
    else if (e == EV_BACK_PRESS) go_back();
    else if (e == EV_ENC_PRESS) {
      if (cur == 0) td_save();
      else if (cur == 1) td_discard();
      else go_back();
    }
    break;
  case SCR_AL_LIST:
    ...
    break;
  case SCR_AL_MENU:
    if (e == EV_ENC_CW) move(+1, 7);
    else if (e == EV_ENC_CCW) move(-1, 7);
    else if (e == EV_BACK_PRESS) al_request_leave();
    else if (e == EV_ENC_PRESS) {
      switch (cur) {
        case 0: al.enabled = !al.enabled; break;
        case 1: go(SCR_AL_DATEOPT); break;
        case 2: go(SCR_AL_SOUND); break;
        case 3: go(SCR_AL_LIGHT); break;
        case 4: go(SCR_AL_SNOOZE); break;
        case 5: al_save(); break;
        case 6: al_discard(); break;
      }
    }
    break;
  case SCR_AL_DATEOPT:
    if (e == EV_ENC_CW) move(+1, 2);
    else if (e == EV_ENC_CCW) move(-1, 2);
    else if (e == EV_BACK_PRESS) go_back();
    else if (e == EV_ENC_PRESS) {
      al.daily = cur == 0;
    ...
    break;
  case SCR_AL_DATE:
    if (edit_common(e, al_date_step, al_snap_restore)) break;
    if (e == EV_ENC_CW) move(+1, 4);
    else if (e == EV_ENC_CCW) move(-1, 4);
    else if (e == EV_BACK_PRESS) go_back();
    else if (e == EV_ENC_PRESS) {
      if (cur < 3) { al_snap_save(); editing = true; }
      else go(SCR_AL_TIME);
    }
    break;
  case SCR_AL_TIME: {
    if (edit_common(e, al_time_step, al_snap_restore)) break;
    uint8_t nf = al_time_fields();
    if (e == EV_ENC_CW) move(+1, nf + 1);
    else if (e == EV_ENC_CCW) move(-1, nf + 1);
    else if (e == EV_BACK_PRESS) go_back();
    else if (e == EV_ENC_PRESS) {
      if (cur < nf) { al_snap_save(); editing = true; }
      else pop_to(SCR_AL_MENU);
    }
    break;
  }
  case SCR_AL_SOUND:
    ...
    else if (e == EV_BACK_PRESS) { hw_test_stop(); go_back(); }
    else if (e == EV_ENC_PRESS) { sel_sound = cur + 1; go(SCR_AL_SOUND_OPT); }
    break;
  case SCR_AL_SOUND_OPT:
    if (e == EV_ENC_CW) move(+1, 2);
      ...
      if (cur == 0) {
        if (!hw_sound_ok()) { ui_output_error(true); break; }
        al.sound = sel_sound; hw_test_stop();
        char b[24]; snprintf(b, sizeof(b), "Sound %u selected", sel_sound);
        go_back(); msg_show(b, "Save to keep it", SCR_AL_SOUND, sel_sound - 1);
      } else if (!hw_sound_test(sel_sound)) ui_output_error(true);
    }
    break;
  case SCR_AL_LIGHT:
    if (e == EV_ENC_CW) move(+1, 3);
    ...
    else if (e == EV_ENC_PRESS) { sel_light = cur + 1; go(SCR_AL_LIGHT_OPT); }
    break;
  case SCR_AL_LIGHT_OPT:
    if (e == EV_ENC_CW) move(+1, 2);
      ...
      if (cur == 0) {
        if (!hw_light_ok()) { ui_output_error(false); break; }
        al.light = sel_light; hw_test_stop();
        char b[24]; snprintf(b, sizeof(b), "Light %u selected", sel_light);
        go_back(); msg_show(b, "Save to keep it", SCR_AL_LIGHT, sel_light - 1);
      } else if (!hw_light_test(sel_light)) ui_output_error(false);
    }
    break;
  case SCR_AL_SNOOZE:
    if (edit_common(e, al_snooze_step, al_snap_restore)) break;
    if (e == EV_ENC_CW) move(+1, 3);
    else if (e == EV_ENC_CCW) move(-1, 3);
    else if (e == EV_ENC_PRESS) { al_snap_save(); editing = true; }
    else if (e == EV_BACK_PRESS) go_back();
    break;
  case SCR_AL_CONFIRM:
    if (e == EV_ENC_CW) move(+1, 3);
  ...
  case SCR_BRIGHT:
    ...
    else if (e == EV_BACK_PRESS) bright_request_leave();
    else if (e == EV_ENC_PRESS) {
      switch (cur) {
        case 0: br_auto_draft = true; break;
        case 1: br_auto_draft = false; break;
        case 2: case 3: case 4: br_auto_draft = false; br_level_draft = cur - 1; break;
        case 5: bright_save(); break;
        case 6: bright_discard(); break;
      }
    }
    break;
  ...
  case SCR_BRIGHT_CONFIRM:
    if (e == EV_ENC_CW) move(+1, 3);
      ...
      if (cur == 0) bright_save();
      else if (cur == 1) bright_discard();
      else go_back();
    }
    break;
  ...
  case SCR_DST:
    ...
    else if (e == EV_BACK_PRESS) dst_request_leave();
    else if (e == EV_ENC_PRESS) {
      switch (cur) {
        case 0: dst_auto_draft = true; dst_direction = 0; break;
        case 1: dst_auto_draft = false; dst_direction = 1; break;
        case 2: dst_auto_draft = false; dst_direction = -1; break;
        case 3: dst_auto_draft = false; dst_direction = 0; dst_hr_draft = 0; break;
        case 4: dst_save(); break;
      }
    }
    break;
  ...
  case SCR_DST_CONFIRM:
    if (e == EV_ENC_CW) move(+1, 3);
      ...
      if (cur == 0) dst_save();
      else if (cur == 1) dst_discard();
      else go_back();
    }
    break;

  case SCR_TD_REVIEW:
    if (e == EV_BACK_PRESS) go_replace(SCR_TD_MENU);
    else if (e == EV_ENC_CW || e == EV_ENC_CCW) move(1, 2);
    else if (e == EV_ENC_PRESS) {
      if (cur == 0) go_replace(SCR_TD_MENU);
      else {
        const DateTime &v = td_result.proposed;
        td.year = v.year(); td.month = v.month(); td.day = v.day();
        td.hour = v.hour(); td.minute = v.minute(); td.second = v.second();
        td_request.local = v;
        td_request.groups &= ~td_result.committed;
        if (td_request.groups & (SAVE_TIME | SAVE_DATE)) td_request.groups |= SAVE_TIME | SAVE_DATE;
...
void ui_alarm_overlay(bool on) {
  overlay = on;
  if (on) snooze_countdown = false;
  if (!on && scr == SCR_MSG) msg_until = millis() + MSG_MS;
}
void ui_snooze_refused() { snooze_notice_until = millis() + MSG_MS; }
void ui_snoozed() {
  uint8_t index; uint32_t seconds;
  ...
  if (scr == SCR_TD_SAVING) {
    ...
    if (td_accepted && !overlay && now - td_started >= 400) {
      if (td_result.status == SaveStatus::Success || td_result.status == SaveStatus::Error) {
        ...
        if (committed & SAVE_TIME) { td_orig.hour = td.hour; td_orig.minute = td.minute; td_orig.second = td.second; }
        if (committed & SAVE_DATE) { td_orig.year = td.year; td_orig.month = td.month; td_orig.day = td.day; }
        if (committed & SAVE_FORMAT) td_orig.fmt24h = td.fmt24h;
        if (td_result.status == SaveStatus::Success) {
          td_orig = td; pop_to(SCR_MENU); msg_show("Saved", nullptr, SCR_MENU, cur);
        } else {
          go_replace(SCR_TD_MENU);
          msg_here(committed & (SAVE_TIME | SAVE_DATE) ? "Time set" : "Save failed", td_result.detail);
        }
...
static void draw_row(uint8_t slot, const char *s, bool hi) {
  ...
  if (hi) { oled.drawBox(0, y - 9, 128, 11); oled.setDrawColor(0); }
  oled.drawStr(2, y, s);
  oled.setDrawColor(1);
}
static void draw_list(const char *t, const char *const *items, uint8_t n) {
  ...
  if (n > 4) {
    ...
    uint8_t h = (uint8_t)(44u * 4u / n); if (h < 5) h = 5;
...
static void draw_field(uint8_t x, uint8_t y, const char *s, bool hi, bool act) {
  ...
  if (act) {
    ...
    oled.setDrawColor(0); oled.drawStr(x, y, s); oled.setDrawColor(1);
  } else {
    oled.drawStr(x, y, s);
    if (hi) oled.drawHLine(x - 3, y + 3, w + 6);
  }
}
static void fmt_hm(char *b, size_t n, uint8_t h, uint8_t m, bool f24) {
  if (f24) snprintf(b, n, "%02u:%02u", h, m);
  else snprintf(b, n, "%u:%02u %s", hour12(h), m, is_pm(h) ? "PM" : "AM");
}
...
static void draw_home_alarm_status(const Settings &settings) {
  ...
  uint8_t count = 0;
  for (const auto &alarm : settings.alarm) if (alarm.enabled) ++count;
  if (!count) {
  ...
  if (count == 3) oled.setFont(u8g2_font_4x6_tf);
  uint8_t slot = 0;
  for (uint8_t i = 0; i < 3; ++i) {
    const Alarm &a = settings.alarm[i];
    if (!a.enabled) continue;
    if (settings.fmt24h) snprintf(buf, sizeof(buf), "%u. %02u:%02u", i + 1, a.hour, a.minute);
    else snprintf(buf, sizeof(buf), "%u. %u:%02u%s", i + 1, hour12(a.hour), a.minute, is_pm(a.hour) ? "pm" : "am");
...
static void draw_home(const DateTime &t, const Settings &settings = cfg) {
  ...
  snprintf(buf, sizeof(buf), "%s %04u-%02u-%02u",
           weekday_short(t.dayOfTheWeek()), t.year(), t.month(), t.day());
  oled.drawStr(2, 10, buf);
  if (hw_dst_offset()) {
    oled.setFont(u8g2_font_5x7_tf); oled.drawStr(111, 9, "DST");
  }
  if (settings.fmt24h) snprintf(buf, sizeof(buf), "%02u:%02u", t.hour(), t.minute());
  else snprintf(buf, sizeof(buf), "%u:%02u", hour12(t.hour()), t.minute());
  ...
  if (!settings.fmt24h) oled.drawStr(x + w + 4, 26, is_pm(t.hour()) ? "PM" : "AM");
...
void ui_home_preview(uint8_t enabled_mask, bool fmt24h) {
  ...
  for (uint8_t i = 0; i < 3; ++i) preview.alarm[i].enabled = (enabled_mask & (1 << i)) != 0;
...
static void draw_time_fields(const char *t, uint8_t hh, uint8_t mm,
                             const uint8_t *ss, bool f24,
  ...
  oled.setFont(u8g2_font_6x12_tf);
  if (extra) draw_row(3, extra, cur == nf);
  else       oled.drawStr(2, 62, editing ? "Turn to change" : "Press to edit");
}

// ---- 5 / 10. date field list -----------------------------------------
static void draw_date_fields(const char *t, uint16_t yy, uint8_t mo, uint8_t dd,
                             bool show_adjusted, const char *extra) {
  draw_title(t);
  char b[8];
  oled.setFont(u8g2_font_9x15_tr);
  ...
  oled.setFont(u8g2_font_6x12_tf);
  if (show_adjusted) oled.drawStr(2, 46, "Day adjusted");
  else               oled.drawStr(2, 46, weekday_name(weekday_of(yy, mo, dd)));
  if (extra) draw_row(3, extra, cur == 3);
}
...
static void draw_ringing() {
  ...
  uint8_t left = (a.snooze_max > hw_alarm_snoozes_used())
                 ? (uint8_t)(a.snooze_max - hw_alarm_snoozes_used()) : 0;
  if (left) snprintf(b, sizeof(b), "Snooze x%u   Back=off", left);
  else      snprintf(b, sizeof(b), "No snoozes  Back=off");
  oled.drawStr(2, 62, b);
  if (int32_t(snooze_notice_until - millis()) > 0) oled.drawStr(2, 46, "Snooze limit reached");
  else if (!hw_sound_ok()) oled.drawStr(2, 46, "Sound output error");
  else if (!hw_light_ok()) oled.drawStr(2, 46, "Lighting HW error");
}

static bool draw_snoozed() {
  uint8_t index; uint32_t seconds;
  if (!next_snooze(index, seconds)) return false;
  draw_title("Snoozed");
  uint8_t row = 0;
  for (uint8_t i = 0; i < 3; ++i) {
    if (!hw_snooze_remaining(i, seconds)) continue;
    char b[24];
    snprintf(b, sizeof(b), "Alarm %u: %02lu:%02lu left", i + 1,
...
void ui_draw(const DateTime &local) {
  ...
  if (overlay) { draw_ringing(); hw_display_present(); return; }
  if (snooze_countdown) {
    if (draw_snoozed()) { hw_display_present(); return; }
    snooze_countdown = false;
  }

  char r[7][22];
  const char *items[7];
  for (uint8_t i = 0; i < 7; i++) items[i] = r[i];

  switch (scr) {

  case SCR_HOME: draw_home(local); break;
  ...
  case SCR_TD_MENU: {
    char t[14]; fmt_hm(t, sizeof(t), td.hour, td.minute, td.fmt24h);
  ...
  case SCR_AL_LIST:
    for (uint8_t i = 0; i < 3; i++) {
      char t[12]; fmt_hm(t, sizeof(t), cfg.alarm[i].hour, cfg.alarm[i].minute, cfg.fmt24h);
      snprintf(r[i], 22, "Alarm %u  %-8s %s", i + 1, t,
               cfg.alarm[i].enabled ? "on" : "off");
    }
  ...
  case SCR_AL_MENU: {
    ...
    if (al.daily) snprintf(r[1], 22, "Time: daily");
    else          snprintf(r[1], 22, "Time: %02u-%02u", al.month, al.day);
    ...
    char t[22]; snprintf(t, sizeof(t), "Alarm %u", sel_alarm + 1);
    draw_list(t, items, 7);
    break;
  }
  ...
  case SCR_AL_TIME:
    draw_time_fields("Alarm time", al.hour, al.minute, nullptr, cfg.fmt24h,
                     al_time_fields(), "Done");
    break;
  ...
  case SCR_AL_SOUND_OPT: {
    ...
    char t[22]; snprintf(t, sizeof(t), "Sound %u", sel_sound);
    draw_list(t, items, 2);
    break;
  }
  ...
  case SCR_AL_LIGHT_OPT: {
    ...
    char t[22]; snprintf(t, sizeof(t), "Light %u", sel_light);
    draw_list(t, items, 2);
    break;
  }
  ...
  case SCR_AL_SNOOZE: {
    ...
    snprintf(r[0], 22, "Delay     %s%u%s min", cur == 0 ? o : " ", al.snooze_min, cur == 0 ? c : " ");
    snprintf(r[1], 22, "Max snooze%s%u%s",     cur == 1 ? o : " ", al.snooze_max, cur == 1 ? c : " ");
    if (al.length_min == 0)
      snprintf(r[2], 22, "Ring: %s0%s indefinite", cur == 2 ? o : " ", cur == 2 ? c : " ");
    else
      snprintf(r[2], 22, "Ring for  %s%u%s min", cur == 2 ? o : " ", al.length_min, cur == 2 ? c : " ");
  ...
  case SCR_BRIGHT: {
    ...
    for (uint8_t i = 0; i < 3; i++)
      snprintf(r[2 + i], 22, "   %c %u %s",
               (!br_auto_draft && br_level_draft == i + 1) ? '*' : ' ', i + 1, lv[i]);
  ...
  case SCR_DST: {
    ...
    snprintf(r[3], 22, "[%c] Off", !dst_auto_draft && !dst_direction && dst_hr_draft == 0 ? 'x' : ' ');
  ...
  case SCR_TD_REVIEW: {
    ...
    const DateTime &v = td_result.proposed;
    snprintf(r[0], 22, "%04u-%02u-%02u %02u:%02u", v.year(), v.month(), v.day(), v.hour(), v.minute());
    oled.drawStr(2, 24, r[0]);
    oled.drawStr(2, 35, td_result.status == SaveStatus::Unknown ? "Current clock" : "DST: standard choice");
  ...
  case SCR_MSG:
    ...
    if (msg_l2[0]) oled.drawStr((int)(64 - oled.getStrWidth(msg_l2) / 2), 44, msg_l2);
    break;
  }
...
void ui_begin() {
  scr = SCR_HOME; cur = 3; nav_sp = 0; editing = false; overlay = false;
  snooze_countdown = false;
}
...
void ui_format_selftest() {
  ...
  auto check = [&](bool ok, const char *name) {
    ++checks; if (!ok) { ++failed; Serial.printf("[test] FAIL %s\n", name); }
  };
  struct Example { uint8_t hour, minute; const char *f12, *f24; };
  const Example examples[] = {
    {0, 0, "12:00 AM", "00:00"}, {12, 0, "12:00 PM", "12:00"},
    {7, 5, "7:05 AM", "07:05"}, {19, 5, "7:05 PM", "19:05"},
