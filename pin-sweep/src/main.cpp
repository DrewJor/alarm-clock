...
void setup() {
  ...
  (void)analogRead(LDR);
  analogSetPinAttenuation(LDR, ADC_11db);
  (void)analogRead(LDR);
  Serial.printf("  after ADC init, no pull ................. %u\n", avg());

  rtc_gpio_pullup_dis((gpio_num_t)LDR);
  rtc_gpio_pulldown_en((gpio_num_t)LDR);
  delay(10);
  Serial.printf("  rtc_gpio_pulldown_en .................... %u\n", avg());

  rtc_gpio_pulldown_dis((gpio_num_t)LDR);
  rtc_gpio_pullup_en((gpio_num_t)LDR);
  delay(10);
  Serial.printf("  rtc_gpio_pullup_en ...................... %u\n", avg());

  rtc_gpio_pullup_dis((gpio_num_t)LDR);
  gpio_pulldown_en((gpio_num_t)LDR);
  delay(10);
